...
class LearningPathCuratorAgent:
    ...
    def run(self, role: str, certification: str) -> AgentResponse:
        ...
        match = certs[
            (certs["primary_role"].str.lower() == role.lower())
            & (certs["certification"].str.lower() == certification.lower())
        ]

        if match.empty:
            return AgentResponse(
                self.name,
                "I don't know based on the provided synthetic certification data."
            )
        ...
        answer = (
            ...
            + ", ".join(skills)
            + f". Recommended study hours: {row['recommended_hours']}. "
            + f"Target practice score: {row['target_practice_score']}%."
        )
...
class StudyPlanGeneratorAgent:
    ...
    def run(self, learner_id: str) -> AgentResponse:
        ...
        if learner.empty:
            return AgentResponse(
                self.name,
                "I don't know based on the provided synthetic learner data."
            )
        ...
        remaining_hours = max(
            0,
            int(cert_row["recommended_hours"]) - int(learner_row["hours_studied"])
        )
        ...
        answer = (
            ...
            f"Preferred learning slot: {workload_row['preferred_learning_slot']}. "
            f"{pacing_note}"
        )
...
class ManagerInsightsAgent:
    ...
    def run(self, team_id: str) -> AgentResponse:
        ...
        if team.empty:
            return AgentResponse(
                self.name,
                "I don't know based on the provided synthetic team data."
            )
        ...
        for _, row in at_risk.iterrows():
            ...
            risk_lines.append(
                f"{row['learner_id']} ({row['role']}, {row['certification']}): "
                + ", ".join(reason)
            )

        if risk_lines:
            action = (
                "Recommended manager action: review workload, protect focus time, "
                "and schedule a checkpoint assessment."
            )
        else:
            action = (
                "Recommended manager action: keep current plan and schedule final mock assessments."
            )

        answer = (
            f"{team_id} readiness summary: {len(ready)} ready learner(s), "
            f"{len(at_risk)} at-risk learner(s). "
...
class AssessmentAgent:
    ...
    def run(self, learner_id: str) -> AgentResponse:
        learners = pd.read_csv(DATA_DIR / "learners.csv")
        progress = pd.read_csv(DATA_DIR / "learner_progress.csv")
        certs = pd.read_csv(DATA_DIR / "certification_requirements.csv")
        ...
        learner_row = learner.iloc[0]
        progress_row = progress[progress["learner_id"] == learner_id].iloc[0]
        cert_row = certs[certs["certification"] == learner_row["certification"]].iloc[0]

        certification = learner_row["certification"]
        score = int(learner_row["practice_score_avg"])
        target_score = int(cert_row["target_practice_score"])

        question_bank = {
            "AZ-204": [
                "What is the purpose of Azure Functions in an application architecture?",
                "How should a developer protect an API endpoint?",
                "When would Azure Storage be used in a cloud application?"
            ],
            "AZ-400": [
                "What is the purpose of a CI/CD pipeline?",
                "How can GitHub Actions support release automation?",
                "Why is monitoring important after deployment?"
            ],
            "DP-203": [
                "What is the role of a data pipeline?",
                "Why is monitoring important in data engineering?",
                "How can a data lake support analytics workloads?"
            ],
            "AI-102": [
                "Why is Responsible AI important in an AI solution?",
                "What is the purpose of evaluating an AI application?",
                "How can prompt flow improve AI application development?"
            ]
        }

        questions = question_bank.get(
            certification,
            ["No grounded practice questions are available for this certification."]
        )

        if score >= target_score:
            readiness = "Ready for final mock assessment."
        elif score >= target_score - 5:
            readiness = "Nearly ready. Needs focused revision before exam scheduling."
        else:
            readiness = "At risk. Needs more practice before exam scheduling."

        answer = (
            f"Assessment summary for learner {learner_id} preparing for {certification}: "
            f"Current practice score is {score}%, target score is {target_score}%. "
            f"Readiness decision: {readiness} "
            f"Pending modules: {progress_row['pending_modules']}. "
            f"Grounded practice questions: "
            f"1) {questions[0]} "
            f"2) {questions[1]} "
            f"3) {questions[2]}"
        )
...
class OrchestratorAgent:
    ...
    def route(self, question: str) -> AgentResponse:
        ...
        if "assessment" in q or "practice" in q or "quiz" in q or "evaluate" in q:
            learner_id = "L-1001"
            ...
            return self.assessment_agent.run(learner_id)
        ...
        return AgentResponse(
            self.name,
            "I don't know based on the provided synthetic data. "
            "Try asking about a team, learner, study plan, assessment, or certification."
        )